...
var globalResponseHandler = function (done) {
    return function (err, res, body) {
        ...
        if (err || !res) {
            ...
            } // because body can be empty or malformed
...
var parseTranslations = function (originalStrings, done) {
    return function (err, data) {
        ...
        originalStrings.forEach(function (s, i) {
            if (data[i])
                _.extend(data[i], {originalText: s});
        });
...
var parseSupportedLanguages = function (done) {
    return function (err, languages) {
        ...
        if (languages[0] && !languages[0].name)
            languages = _.pluck(languages, 'language');
        done(null, languages);
    };
};
...
module.exports = function (apiKey, newConcurrentLimit) {
    ...
    api.translate = function (strings, sourceLang, targetLang, done) {
        if (typeof strings !== 'string' && !Array.isArray(strings))
            return done('Input source must be a string or array of strings');
        if (typeof sourceLang !== 'string')
            return done('No target language specified. Must be a string');
        ...
        if (!_.isFunction(done))
            return console.log('No callback defined');
